...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.owner.set`, `agent.owner.clear`, `agent.group.set`, `agent.group.collapse`, `agent.view.set`, `agent.view.clear` |
...
`agent.start` accepts optional `owner` and `caller_pane_id` params; when the caller pane hosts a live agent, the started agent records it as its durable owner. `agent.owner.set` adopts or transfers an agent to a new current owner and `agent.owner.clear` releases it; both return the updated agent record. Agent records expose `agent_id` (a durable identity, never a pane id) and an `ownership` object with immutable `origin`, transferable `current`, and an `orphaned` flag for owner references that no longer resolve.
