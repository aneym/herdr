...
herdr agent owner set <target> <owner>
herdr agent owner clear <target>
herdr agent group hands-on <target>
...
When `agent start` runs inside a pane that hosts a live agent, the started agent is recorded as owned by that caller. `--owner TARGET` records a specific owner instead, and `--no-owner` keeps the new agent a root. `agent owner set` adopts or transfers an agent to a new current owner, and `agent owner clear` releases it to a root; the immutable origin owner recorded when ownership first appeared never changes. Self-ownership and ownership cycles are rejected. See [Agents](/docs/agents/) for the ownership model, orphan handling, and sidebar behavior.

`agent group` arranges the sidebar without touching ownership. `hands-on` (alias `pin`) keeps an agent top-level and visible outside any collapsed owner or orchestrator group; `under` nests it beneath another agent; `auto` (alias `clear`) removes the explicit placement; `collapse` and `expand` fold or open the group the target owns. Self-nesting and cycles are rejected, and a parent that later disappears leaves the agent marked orphaned rather than silently moved. Each command returns the updated agent, whose `group` object reports `placement`, `parent`, `orphaned`, and `collapsed`.
