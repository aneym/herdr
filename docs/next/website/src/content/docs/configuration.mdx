...
Optional actions are unset by default. Bind them with `prefix+` for prefix-mode behavior, or use an explicit modified chord when you intentionally want a direct shortcut. This includes `last_pane` for one-deep toggling and the browser-style `focus_back` and `focus_forward` history actions. For example, tmux-style one-keystroke pane resizing without entering resize mode:
...
Set `max_visible` under `[ui.sidebar.spaces]` to a positive number to fit the spaces section to its content while showing at most that many entries. Additional spaces remain available by scrolling, and the agents section uses the remaining height. The default `0` keeps the draggable ratio split.
