...
Optional actions are unset by default. Bind them with `prefix+` for prefix-mode behavior, or use an explicit modified chord when you intentionally want a direct shortcut. This includes `last_pane` for one-deep toggling and the browser-style `focus_back` and `focus_forward` history actions.
