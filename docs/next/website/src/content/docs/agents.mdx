...
In the sidebar, owner rows show a chevron that collapses or expands their subtree. A collapsed group shows how many agents it hides, one status glyph per hidden agent, and inherits their highest-attention status color, so blocked or unseen work stays visible. Collapse state persists across restarts.
