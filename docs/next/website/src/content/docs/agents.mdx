...
Agents can durably belong to another agent. When an agent runs `herdr agent start` from inside its own pane, the started agent is automatically recorded as owned by the caller and the sidebar nests it beneath its owner. Ownership is anchored to a durable agent identity, not a pane ID, so it survives renames, pane moves between tabs and workspaces, Herdr restarts, and resumable agent sessions.

Each owned agent keeps two references: an immutable origin owner recorded when ownership first appeared, and a transferable current owner. Change the current owner explicitly:

```bash
herdr agent owner set worker lead     # adopt or transfer
herdr agent owner clear worker        # release to a root agent
```

`agent start` accepts `--owner TARGET` to record a specific owner and `--no-owner` to keep the new agent a root. `agent list` and `agent get` report `agent_id` and an `ownership` object with `origin`, `current`, and `orphaned`.

When a current owner can no longer be resolved — for example the owner exited and its session was not resumed — the owned agent is marked orphaned rather than silently flattened. Orphaned agents carry a `◌` marker in the sidebar and stay actionable: adopt them with `agent owner set` or release them with `agent owner clear`. If the owner's agent session comes back elsewhere, the reference reconciles automatically through the recorded session identity.

In the sidebar, owner rows show a chevron that collapses or expands their subtree. A collapsed group shows how many agents it hides and inherits their highest-attention status color, so blocked or unseen work stays visible. Collapse state persists across restarts.

Sidebar placement can also be set by hand, independently of ownership. A placement never changes who an agent belongs to, what it may do, or where `agent start` records its owner; it only decides where the row sits in the sidebar:

```bash
herdr agent group hands-on reviewer   # pin top-level, outside any collapsed group
herdr agent group under worker lead   # nest beneath lead without adopting it
herdr agent group auto worker         # back to ownership-driven placement
herdr agent group collapse lead       # fold the group lead owns
herdr agent group expand lead
```

A hands-on agent always stays a top-level row with a `⚲` pin marker, so it is never folded into an owner group or an orchestrator group and stays in the agent cycle. An explicit `under` parent wins over the owner edge and over orchestrator adoption; if that parent later disappears, the row falls back to automatic placement and carries the `◌` marker instead of vanishing. Self-nesting and cycles are rejected. `agent list` and `agent get` report a `group` object with `placement` (`auto`, `hands_on`, or `under`), the resolved `parent`, `orphaned`, and `collapsed`. Placement survives pane moves, restarts, and session handoff like ownership does; it ends with the agent occupancy.

The same controls live in the sidebar: right-click an agent row for **Pin hands-on**, **Nest under...** (a picker of the workspace's other agents), **Clear nesting**, and **Collapse group** or **Expand group** on group owners. In the tree view with the agents layer hidden, the tab header that stands in for a group owner carries the group's `+N` count and chevron, so a folded group can still be opened from there. The `keys.toggle_agent_group` and `keys.toggle_hands_on` bindings act on the focused agent from the keyboard; both are unset by default.
