...
pub struct AppState {
    ...
    // PORT-0.9: right-click passthrough gesture tracking is client-shell input
    // state now (src/client/shell/mouse.rs); the server keeps only the config.
...
impl AppState {
    ...
    // PORT-0.9: consumed by the fork's local render loop; the 0.9 server drives
    // pane surfaces from the client shell. (docs/fork/port-0.9/PORT.md)
    #[allow(dead_code)]
    ...
    // PORT-0.9: host mouse capture is decided by the client shell in 0.9
    // (src/client/shell/mouse.rs, src/client/mod.rs). (docs/fork/port-0.9/PORT.md)
