...
pub(crate) const ANIMATION_INTERVAL: Duration = Duration::from_millis(128);
...
pub struct App {
    ...
    /// Consecutive rapid clicks at the same cell: 2 = word, 3 = line.
    pub(crate) pane_click_streak: u8,
...
impl App {
    pub fn new(
        ...
        state.drag = None;
        state.workspace_presses = HashMap::new();
        state.tab_presses = HashMap::new();
        ...
        state.headless_size = config.headless_size();
        ...
        state.pane_outer_borders = config.ui.pane_outer_borders;
        state.pane_scrollbars = config.ui.pane_scrollbars;
        ...
        state.tab_bar_right = Vec::new();
        state.tab_bar_right_separator = String::new();
    ...
    fn apply_live_config(
        ...
        if !invalid_section("ui") {
            ...
            if let Some(diagnostic) = config.invalid_sidebar_bounds_diagnostic() {
                ...
                if self.state.attention_read != config.ui.attention_read {
                    self.state.deferred_attention_read = None;
                }
                self.state.attention_read = config.ui.attention_read;
