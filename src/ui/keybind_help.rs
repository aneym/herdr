...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    let workspace_tab = vec![
        ...
        help_entry(keybind_label(&kb.search), "search sessions"),
    ...
    let panes = vec![
        ...
        help_entry(keybind_label(&kb.focus_back), "focus back"),
        help_entry(keybind_label(&kb.focus_forward), "focus forward"),
    ];
